...
app.post('/api/webhooks/send', async (req, res) => {
  try {
    ...
    const result = await webhookDelivery.deliverWebhook(targetUrl, payload, options);
...
app.post('/api/payroll/events/completed', async (req, res) => {
  try {
    ...
    if (webhookUrl) {
      await webhookDelivery.deliverPayrollEvent(webhookUrl, 'completed', payrollData);
    }
...
app.post('/api/payroll/events/failed', async (req, res) => {
  try {
    ...
    if (webhookUrl) {
      await webhookDelivery.deliverPayrollEvent(webhookUrl, 'failed', payrollData);
    }
